...
public class Incinerator : IsInteractable {
    ...
    public override void UnDo(GameObject player, Vector3 lookingDirection) {
        if (door.transform.localScale == Vector3.zero) StartCoroutine(OpenOrCloseDoor(1));
    }
    ...
    private void OnTriggerEnter(Collider other) {
        ...
        if (other.gameObject.CompareTag("Cube") && Script) Script.Do(gameObject, Vector3.zero);
